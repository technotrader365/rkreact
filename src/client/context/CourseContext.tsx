...
export const CourseProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchData = async () => {
    if (!serviceNow.isConnected()) {
      console.log("Using Mock Data (ServiceNow not configured)");
    ...
    try {
      ...
      console.error("Failed to sync with ServiceNow", err);
      // Fallback to mock on error is risky if partial data, but safe for demo
      // setCourses(initialCourses); 
    } finally {
      setLoading(false);
    }

...
export const Settings: React.FC = () => {
  const [config, setConfig] = useState({ instance: '', username: '', token: '' });
  const [saved, setSaved] = useState(false);
  const [testStatus, setTestStatus] = useState<'idle' | 'testing' | 'success' | 'failed'>('idle');

  useEffect(() => {
    const existing = localStorage.getItem('sn_config');
    if (existing) setConfig(JSON.parse(existing));
  }, []);
      ...
      <form onSubmit={handleSave} className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 space-y-6">
        ...
        >
          {testStatus === 'testing' ? 'Testing Connection...' : 
           ...
           testStatus === 'failed' ? 'Connection Failed (Check Creds)' : 'Connect to ServiceNow'}
        </button>
